...
import { formateReceivedGraphData } from "./utils/received-payment/formateReceivedGraphData";
import SelectYearDropdown from "./component/SelectYearDropdown";

const ReceivedPayment = () => {
  const { data, isLoading } = useDashboardReceivedPaymentQuery();
  const [year, setYear] = useState<number | undefined>(undefined);

  const { graphData, totalReceived, maxValue, yearTotal } = useMemo(() => {
    if (!isLoading && data.statusCode === 200) {
      return formateReceivedGraphData(data.data, year);
    } else {
      return {
        maxValue: 4,
        totalReceived: 0,
    ...
    <View style={styles.root}>
      ...
      <Flex flexWrap="wrap" my={10}>
        <PaymentLabel
          ...
          background="#00FFFF"
          rupeeIcon="rupeeGraphBlue"
        />
        {year && (
          <PaymentLabel
            amount={yearTotal}
            label={year + " Received Payment"}
            background="#00FFFF"
            rupeeIcon="rupeeGraphBlue"
          />
        )}
...
export default memo(ReceivedPayment);
