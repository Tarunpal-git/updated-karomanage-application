...
import { useDeleteBulkDataMutation } from "../../../../../apis/hooks/upload-forms/mutation/useDeleteBulkData.mutation";

interface IUpdateStatusModal {
  isVisible: boolean;
  handleClose: () => void;
  refetch: () => void;
  data: TUploadFormTemplate;
}
...
const UpdateStatusModal: FC<IUpdateStatusModal> = ({
  ...
  const { mutateAsync, isPending } = useUpdateUploadFormStatusMutation();
  const { mutateAsync: deleteBulkData, isPending: deleteLoading } =
    useDeleteBulkDataMutation();
  ...
  const updateStatus = async (
    ...
    const res =
      values.status === "delete"
        ? await deleteBulkData({ formTemplateId: data.formTemplateId })
        : await mutateAsync({
            templateData: { ...data, formStatus: values.status },
          });
        ...
        <Flex styles={styles.modalView} flexDirection="column">
          <Flex my={20}>
            <ScalableText fontFamily="Medium">
              Change the status of this form
            </ScalableText>
          </Flex>
          ...
          <Flex mt={20}>
            ...
            <Button
              ...
              loading={isPending || deleteLoading}
