...
import { useUpdateFollowUpMutation } from "../../../../../apis/hooks/enquiry/mutation/useUpdateFollowUp.mutation";
...
const EditFollowUpModal: FC<IEditFollowUpModal> = ({
  ...
  const { isPending, mutateAsync } = useUpdateFollowUpMutation();
  ...
  const { flags, isLoading, error } = useDynamicFlags({
    flag: "form",
    formTemplateId: queryId,
  });
  console.log("[FormEditFollowUpModal] Dynamic Flags:", flags);
  console.log("[FormEditFollowUpModal] Error:", error);

  const onSubmit = async (values: typeof forms.followUp.values) => {
    const promises = [];

    for (const followUpId of selectedQueries) {
      promises.push(
        await mutateAsync({
          ...values,
