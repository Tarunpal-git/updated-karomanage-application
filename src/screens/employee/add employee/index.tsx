...
export default function AddEmployeeScreen() {
  ...
  // Store final referral data (won't be reset until form submit)
  const [savedReferralAmount, setSavedReferralAmount] = useState<number>(0);
  const [savedReferralPaymentStatus, setSavedReferralPaymentStatus] = useState<string>('');
  const [savedReferralPaymentMode, setSavedReferralPaymentMode] = useState<string>('');

  // API Call - Referral Agents
  const { data: agentsData, isLoading: isLoadingAgents, refetch: refetchAgents } = useGetReferralAgentsQuery();
...
// ✅ Reset form only if it's empty (first time) and set latest employee code when screen comes into focus
useFocusEffect(
  useCallback(() => {
    // Check if form already has data - if yes, don't reset (user is coming back)
    const currentValues = handler.getValues();
    const hasData = currentValues.first || currentValues.phone || currentValues.email;
    ...
    // Set latest employee code - only if code field is empty
    if (!isLoadingEmployees && employeesData?.data && !currentValues.code) {
      const nextCode = generateNextEmployeeCode();
      setValue("code", nextCode);
    }
  }, [handler, isLoadingEmployees, employeesData, setValue])
...
const skillList: string[] = employeeProfileData?.data?.skill || [];
  ...
  // Populate experience from route params (for edit mode)
  useEffect(() => {
    const params = route.params as any;
    if (params?.employeeData?.experience) {
      // Convert hyphen to en-dash to match dropdown options
      const experienceValue = params.employeeData.experience.replace(/-/g, '–');
      setExpYear(experienceValue);
    } else if (params?.employeeProfessionalDetails?.releventExperienceYear) {
      // Handle direct API response format
      const experienceValue = params.employeeProfessionalDetails.releventExperienceYear.replace(/-/g, '–');
      setExpYear(experienceValue);
    }
  }, [route.params]);
...
const handleClearPanImage = () => {
  ...
  const handleReferralSubmit = () => {
    ...
    // Store final referral data (won't be reset)
    setSavedReferralAmount(amount);
    setSavedReferralPaymentStatus(referralPaymentStatus);
    setSavedReferralPaymentMode(referralPaymentMode);
    ...
    // Reset form fields and close modal (but keep saved data)
    setReferralAmount('');
    setReferralPaymentStatus('');
    setReferralPaymentMode('');
    setReferralAmountError('');
  ...
  const onSubmit = (d:any)=>{
    ...
    const final = {
      ...
      referredBy,
      referredById,
      referralAmount: savedReferralAmount || 0,
      referralPaymentStatus: savedReferralPaymentStatus || '',
      referralPaymentMode: savedReferralPaymentMode || '',
      department,
      aadharImageUri: aadharImageUri,  // ← Line 3985 me add karo
      panImageUri: panImageUri,  
    };
  ...
  return (
    <SafeView>
      <AppHeader title=      "Create Employee" showDrawer={false}/>
      ...
      <View style={styles.page}>
        <View style={styles.card}>
          ...
          <ThemeScrollView contentContainerStyle={styles.scroll}>
            ...
            {/* ================== FIELDS ================== */}
            <Field label="First Name*" name="first" required control={control} errors={errors}
              rules={{required:"This field is required",pattern:{value:/^[A-Za-z ]+$/,message:"Insert only normal characters"}}}/>
            <Field label="Last Name" name="last" control={control} errors={errors}
              rules={{pattern:{value:/^[A-Za-z ]+$/,message:"Insert only normal characters"}}}/>
            <Field label="Employee Code*" name="code" required control={control} errors={errors}
              rules={{required:"This field is required"}}/>
            <Field label="Phone Number*" name="phone" required keyboard="numeric" control={control} errors={errors}
              rules={{required:"This field is required",pattern:{value:/^[0-9]{10}$/,message:"Enter valid phone number"}}}/>
            <Field label="Email*" name="email" required keyboard="email-address" control={control} errors={errors}
              rules={{required:"This field is required",pattern:{value:/^[^\s@]+@[^\s@]+\.[^\s@]+$/,message:"Enter valid email"}}}/>
            {/* Date of Birth */}
            <Text style={styles.label}>Date of Birth*</Text>
<Controller
  ...
  render={({ field: { value, onChange }, fieldState: { error } }) => (
    <>
      <View style={[styles.drop, error ? { borderColor: "red", borderWidth: 2, marginBottom: 2 } : { borderColor: "#000", borderWidth: 1 }]}>
        <DateInput
          handler={handler} 
          name="dob"
          label="Enter date of birth"
          ...
          maximumDate={new Date(2011, 11, 31)}
          minimumDate={new Date(1950, 0, 1)}
          inputTextStyles={{ fontSize: 13, color: '#777' }}
        />
      </View>
      {error && (
        <Text style={{ color: "red", marginTop: 2, marginBottom: 6, fontSize: 11 }}>
          {error.message}
        </Text>
      )}
    </>
...
/>
            <Field label="Father Name" name="father" control={control} errors={errors}
              rules={{pattern:{value:/^[A-Za-z ]+$/,message:"Insert only normal characters"}}}/>
            <Field label="Father Phone Number" name="fphone" keyboard="numeric" control={control} errors={errors}
              rules={{pattern:{value:/^[0-9]{10}$/,message:"Enter valid phone number"}}}/>
            <Field label="Address" name="address" control={control} errors={errors}
              rules={{minLength:{value:3,message:"Min 3 Character"}}}/>
            {/* DEPARTMENT FIELD */}
<Text style={styles.label}>Department</Text>
...
{deptOpen && (
  ...
  onPress={() => setExpOpen(!expOpen)}
>
  <Text style={{color:"#555",fontSize:14}}>{expYear || "Select experience"}</Text>
...
>
  ...
  <AutoHeightImage source={IMAGES.chevronDownIcon} width={10} />
            ...
            {openReferralDropdown && (
              <View style={styles.refDropdown}>
               {isLoadingAgents ? (
                 <View style={styles.refOption}>
                   <Text style={{ color: "#888" }}>Loading agents...</Text>
                 </View>
               ) : referralList.length === 0 ? (
                 <View style={styles.refOption}>
                   <Text style={{ color: "#888" }}>Data not found</Text>
                 </View>
               ) : (
                 referralList.map((agent: any, i: number) => {
                   // Extract agent name from object
      ...
      <AgentSelectionModal
        ...
        onClose={async ()=>{
          setShowReferralModal(false);
          // Refetch agents list when modal closes to ensure latest data
          await refetchAgents();
        }}
        agents={referralList}
        onAgentSelected={async (id,name)=>{
          // Refetch agents list to get the newly created agent
          await refetchAgents();
          setReferredBy(name);
          setReferredById(id);
          setShowReferralModal(false);
      ...
      <Modal
        ...
        onRequestClose={() => {
          ...
          setDropdownValue({ label: '', value: '' });
          setSelectedAgentForReferral(null);
          // Reset saved referral data on cancel
          setSavedReferralAmount(0);
          setSavedReferralPaymentStatus('');
          setSavedReferralPaymentMode('');
          setReferredBy('');
          setReferredById('');
        }}
      >
      ...
      render={({field:{value,onChange},fieldState:{error}})=>(
        <View>
          <TextInput
            ...
            placeholder={label}
            keyboardType={keyboard}
            style={[styles.input,error && {borderColor:"red",borderWidth:2, marginBottom: 2}]}
          />
          {error && error.message ? (
            <Text style={{color:"red",marginTop:2,marginBottom:6,fontSize:11 }}>
              {error.message}
