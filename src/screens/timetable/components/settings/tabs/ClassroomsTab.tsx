...
const ClassroomsTab = () => {
  ...
  const [showSuccessPopup, setShowSuccessPopup] = useState(false);
  const [successMessage, setSuccessMessage] = useState("");
  const [originalClassroom, setOriginalClassroom] = useState<any>(null);
  ...
  const closeEditModal = () => {
    setEditModalVisible(false);
    setEditingClassroom(null);
    setOriginalClassroom(null);
    resetForm();
  };

  // Check if there are any changes made
  const hasChanges = useMemo(() => {
    // If originalClassroom is not set, no changes can be detected - disable button
    if (!originalClassroom || !editingClassroom) return false;
    ...
    const currentName = normalizeValue(classRoomName);
    const currentCapacity = normalizeValue(capacity);
    const currentLocation = normalizeValue(location);
    
    const originalName = normalizeValue(originalClassroom.classRoomName);
    const originalCapacity = normalizeValue(originalClassroom.capacity);
    const originalLocation = normalizeValue(originalClassroom.location);
    
    // Compare all fields - return true only if ANY field has changed
    const nameChanged = currentName !== originalName;
    const capacityChanged = currentCapacity !== originalCapacity;
    const locationChanged = currentLocation !== originalLocation;
    
    return nameChanged || capacityChanged || locationChanged;
  }, [originalClassroom, editingClassroom, classRoomName, capacity, location]);

  // Explicit check for button disabled state
  const isUpdateButtonDisabled = useMemo(() => {
    // Disable if: no changes, update pending, or originalClassroom not set
    return !hasChanges || updateClassroom.isPending || !originalClassroom || !editingClassroom;
  }, [hasChanges, updateClassroom.isPending, originalClassroom, editingClassroom]);
  ...
  const handleSubmit = () => {
    ...
    if (!classRoomName.trim()) {
      setClassRoomError("This field is required");
      return;
    }
    ...
    createClassroom.mutate(payload, {
      onSuccess: () => {
        ...
        setSuccessMessage("Classroom created successfully!");
        setShowSuccessPopup(true);
        setClassRoomError(""); // reset error
      },
    });
  };
  const handleEdit = (item: any) => {
    setEditingClassroom(item);
    const normalizedName = (item.classRoomName ?? "").trim();
    const normalizedCapacity = item.capacity ? String(item.capacity).trim() : "";
    const normalizedLocation = (item.location ?? "").trim();
    
    setClassRoomName(normalizedName);
    setCapacity(normalizedCapacity);
    setLocation(normalizedLocation);
    setClassRoomError("");
    // Store original normalized values to compare changes
    setOriginalClassroom({
  ...
  const handleUpdate = () => {
    ...
    updateClassroom.mutate(payload, {
      onSuccess: () => {
        ...
        setSuccessMessage("Classroom updated successfully!");
        setShowSuccessPopup(true);
        setClassRoomError(""); // reset
        setOriginalClassroom(null); // reset original values
      },
    });
  };
  ...
  style={[
    styles.input,
    classRoomError ? { borderColor: "red" } : null,
  ]}
  placeholder="Classroom name *"
  value={classRoomName}
  onChangeText={(text) => {
    setClassRoomName(text);
    if (text.trim()) setClassRoomError(""); // typing start hote hi error hata do
  }}
/>
...
{classRoomError ? (
  ...
  onChangeText={(text) => {
    setClassRoomName(text);
    if (text.trim()) setClassRoomError("");
  }}
/>
