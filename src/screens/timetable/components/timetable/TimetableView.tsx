...
const TimetableView = ({ onRequestClassroomsTab }: TimetableViewProps) => {
  ...
  // ⭐ Get courseId from batch details (to fetch course subjects)
  ...
  const courseId = useMemo(() => {
    if (batchDetailsData?.statusCode === 200 && batchDetailsData?.data?.courses?.length > 0) {
      return batchDetailsData.data.courses[0].courseId;
    }
    return "";
  }, [batchDetailsData]);

  // ⭐ Course details API (to get subjects for the selected batch's course)
  ...
  // ⭐ SUBJECT MAP (from course details + course list + timetable slots)
  const subjectMap = useMemo(() => {
    const map: { [key: string]: string } = {};
    
    // Priority 1: Course details API (most accurate for selected batch's course)
    if (courseDetailData?.statusCode === 200 && courseDetailData?.data?.subjects) {
      courseDetailData.data.subjects.forEach((s: any) => {
    ...
    // Priority 2: Course list data (fallback)
    if (courseData?.statusCode === 200) {
      courseData.data.forEach((c: any) => {
        c.subjects?.forEach((s: any) => {
          if (s.subjectId && s.subjectName && !map[s.subjectId]) {
            map[s.subjectId] = s.subjectName;
          }
        });
      });
    ...
    // Priority 3: Extract from timetable slots (if slot has subjectName directly)
    if (timeTableData?.statusCode === 200 && timeTableData?.data?.repeatDays) {
      const repeatDays = timeTableData.data.repeatDays;
      Object.values(repeatDays).forEach((daySlots: any) => {
        if (Array.isArray(daySlots)) {
          daySlots.forEach((slot: any) => {
            if (slot.subjectId && slot.subjectName && !map[slot.subjectId]) {
              map[slot.subjectId] = slot.subjectName;
            }
          });
        }
      });
    ...
    return map;
  }, [courseDetailData, courseData, timeTableData]);
