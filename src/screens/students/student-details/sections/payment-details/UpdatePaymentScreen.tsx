...
//       // Send same key as API: previousFeeSlipReceivedAmount = total received so far (so PDF shows correct "Previous Payments")
//       const previousFeeSlipReceivedAmount = Number(totalReceivedAmount) || 0;
//       const remainingDueAmount = totalDueAmount ?? 0;
//       const netAmountDue = Math.max(0, (courseFee || 0) - previousFeeSlipReceivedAmount - (previousDiscountAmount || 0));
//       const finalAmountDue = Math.max(0, netAmountDue - (discountAmount || 0));
...
//           previousFeeSlipReceivedAmount: previousFeeSlipReceivedAmount,
...
console.log(
  ...
  response?.data?.courses?.[0]?.paymentDetails?.installmentDetails
);
      ...
      if (response.statusCode === 200) {
        ...
        // 📧 YAHAN SE NAYA CODE SHURU - Invoice email bhejne ka code
        if (isPayingFirstInstallment === 'paid') {
          console.log("📧 === SENDING INVOICE EMAIL FOR PAID INSTALLMENT ===");
          
          try {
            const firstInstallment = dynamicInstallments[0];
            const studentDetails = studentData?.data;
            
            if (studentDetails && courseData && organization) {
              const gstRuleData = organization?.gstRuleData;
              const rawInclusionType: string = String(gstRuleData?.inclusionType || "noGST");
              let gstInclusionType: string = "NoGST";
              const normalizedType = rawInclusionType.toLowerCase();
              ...
              const sgstPercentage = gstRuleData?.sgstPercentage || 0;
              const gstinNumber = gstRuleData?.gstinNumber || '';
              const paidAmount = firstInstallment.duePayment || 0;
              const courseFee = paymentDetails?.totalPayment || courseData?.data?.courseFee || courseData?.courseFee || 0;
              ...
              const amountToWords = (amount: number) => `Rupees ${amount.toLocaleString('en-IN')} only`;
              ...
              const receiptNo = Math.floor(Date.now() / 1000) % 100000;
              const totalDueAmount = paymentAfterDiscount - paidAmount;
              const amountAfterDiscount = courseFee - discountAmount;
              ...
              const invoicePayload = {
                ...
                studentFeeSlip: {
                  ...
                  studentFeeSlipCourseName: courseData?.data?.courseName || courseData?.courseName || course.courseName || '',
                  studentFeeSlipCourseId: course.courseId || '',
                  studentFeeSlipInstallmentId: firstInstallment.installmentId,
                  studentFeeSlipPaymentMode: firstInstallment.paymentMode || '',
                  studentFeeSlipTransactionId: firstInstallment.transactionId || '-',
                  studentFeeSlipPaymentRecieverId: firstInstallment.paymentRecieverId || '',
                  studentFeeSlipAmountInWords: amountToWords(paidAmount),
                  studentFeeSlipPurpose: "mail",
                  studentFeeSlipDate: formatDateForInvoice(),
                  studentFeeSlipGSTIN: gstinNumber,
                  studentFeeSlipStudentPhoneNumber: studentDetails.studentContact || '',
                  studentFeeSlipWebsiteUrl: organization?.organizationWebsiteUrl || '',
                  studentFeeSlipStudentAddress: studentDetails.studentAddress || '',
                  ...
                  totalRemainingDueAmount: totalDueAmount,
                  studentFeeSlipDiscountAmount: discountAmount,
                  studentFeeSlipAmountAfterDiscount: amountAfterDiscount,
                  studentFeeSlipPaidAmount: paidAmount,
                  receivedPaymentCGSTAmount: cgstAmount,
              ...
              await sendInvoiceEmail(invoicePayload);
                  ...
                  <Grid>
                    ...
                    return (
                      <Row key={installment.installmentId} style={styles.dataRow}>
                          ...
                          <Flex flexDirection="row" justify="flex-start" align="center">
                            {installment.paymentStatus?.toLowerCase() === 'paid' ? (
                              <TouchableOpacity 
                                ...
                                style={{ opacity: isSendingInvoice ? 0.5 : 1 }}
                              >
                                <AutoHeightImage source={IMAGES.reportsIcon} width={16} />
                              </TouchableOpacity>
                            ) : (
                              <View >
                        ...
                        <Col size={8} style={{ alignItems: 'flex-start', justifyContent: 'center' }}>
                        <Flex flexDirection="row" justify="flex-start" align="center">
                        {(displayStatus === 'due' || displayStatus === 'overdue') ? (
                        <TouchableOpacity 
                        onPress={() => handleReminderMenu(installment.installmentId)}
                        style={styles.reminderButton}
                        >
                        <AutoHeightImage source={IMAGES.mailIconGray} width={16} />
                          ...
                          <View />
                            ...
                            <Flex flexDirection="row" justify="flex-start" align="center">
                            <TouchableOpacity onPress={() => handleEditInstallment(installment)}>
                            <AutoHeightImage source={IMAGES.editIcon} width={19} />
                            </TouchableOpacity>
                          </Flex>
                          </Col>
                         <Col size={8} style={{ alignItems: 'flex-start', justifyContent: 'center' }}>
                         <Flex flexDirection="row" justify="flex-start" align="center">
                        {(installment.paymentStatus?.toLowerCase() === 'paid' || displayStatus === 'paid') ? (
                        <TouchableOpacity onPress={() => handleDownloadInvoice(installment.installmentId)}>
                        <AutoHeightImage source={IMAGES.downloadIcon} width={19} />
                        </TouchableOpacity>
                        ) : (
                       <ScalableText style={styles.dataText} fontFamily="Regular"></ScalableText>
                          )}
                       </Flex>
                      </Col>
                      </Row>
