...
const StudentDetailsScreen = () => {
  ...
  const handleSelectImage = () => {
    launchImageLibrary(
      { 
        mediaType: 'photo', 
        includeBase64: true, // ✅ Add this to get base64 string
        quality: 0.7,
        maxWidth: 1024,
        maxHeight: 1024,
      }, 
      (response) => {
        if (response.assets && response.assets.length > 0) {
          ...
          // ✅ Convert to base64 data URI format for web compatibility
          if (asset.base64) {
            const imageType = asset.type || 'image/jpeg';
            const base64Uri = `data:${imageType};base64,${asset.base64}`;
            setImageUri(base64Uri);
            console.log('✅ Image converted to base64, length:', base64Uri.length);
          } else {
            // Fallback to URI if base64 is not available (shouldn't happen with includeBase64: true)
            console.warn('⚠️ Base64 not available, using URI:', asset.uri);
            setImageUri(asset.uri || '');
          }
        }
      }
